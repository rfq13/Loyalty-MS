      ...
      if (!file.startsWith('.')) {
        ...
        if (!_.isEmpty(checkTS)) {
          ...
          if (!controllerTSExist) {
            const isDir = fs.lstatSync(groupDir).isDirectory()
            isDir && fs.readdirSync(groupDir).forEach((subFile) => {
              const subFilePath = `${file}/${subFile}`

              const subGroupDir = `${groupDir}/${subFile}`
              const subControllerPath = `${subGroupDir}/controller`

              const subControllerTSExist = fs.existsSync(
                `${subControllerPath}.ts`
              )

              if (!subControllerTSExist) {
                const isSubDir = fs.lstatSync(subGroupDir).isDirectory()
                isSubDir && fs.readdirSync(subGroupDir).forEach((sub2File) => {
        ...
        if (checkJS) {
          ...
          if (!controllerJSExist) {
            fs.readdirSync(groupDir).forEach((subFile) => {
              ...
              const subControllerJSExist = fs.existsSync(
                `${subControllerPath}.js`
              )
